...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        synthesize()
# Monkey-patch load_fsspec para usar weights_only=False
original_load_fsspec = TTS.utils.io.load_fsspec
def patched_load_fsspec(*args, **kwargs):
...
DEFAULT_MODEL = os.getenv('TTS_MODEL', 'tts_models/multilingual/multi-dataset/xtts_v2')
DEFAULT_TEXT = os.getenv('TTS_TEXT', 'Olá, esta é uma síntese via docker-compose!')
DEFAULT_OUTPUT = os.getenv('TTS_OUTPUT', 'output/speak_en.wav')
...
def list_speakers(model_name):
    try:
        tts = TTSClass(model_name, gpu=False)
        if (tts):
           print(tts.speakers)
        else:
            print("Nenhum falante disponível ou SpeakerManager não inicializado.")
...
def synthesize(text=None, model_name=DEFAULT_MODEL, output_path=DEFAULT_OUTPUT, speaker="Ana Florence", language="pt", file=None):
    """
    Converte texto em fala usando Coqui TTS localmente.
    """
    if file:
